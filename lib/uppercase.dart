...
class UpperCaseMode extends StatefulWidget {
  // //MainPageからdataのタイムと入力値を受け取る名前を代入する変数を定義
  String name;
  String timeData;
  //受け取る値を上記の変数に代入
  UpperCaseMode({Key? key, required this.name, required this.timeData})
      : super(key: key);

  @override
  UpperCaseModeState createState() =>
      UpperCaseModeState(name: name, timeData: timeData);
}

class UpperCaseModeState extends State<UpperCaseMode> {
  //mainpageから受け取る
  String name;
  String timeData;
  UpperCaseModeState({required this.name, required this.timeData});
  ...
  //順番に表示(A-Z)関数
  void _incrementUpper() {
    setState(() {
      if (_upperList.isNotEmpty) {
        _text = _upperList[0];
        _upperList.removeAt(0);
      }
    });
  }
  ...
  _setPrefItems(userName) async {
    ...
    //データがあるか確認
    var data = prefs.getString('2-$userName') ?? '---';
    // データが無い場合入れる
    if (data == '---') {
      await prefs.setString('2-$userName', _time);
    }

    //タイムのstring型を比較するためにdouble型に変換
    var doubleTimeData = double.parse(data); //double型のデータタイム(data)が出力される。
